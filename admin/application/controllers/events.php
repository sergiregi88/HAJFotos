...
class Events extends CI_Controller {
	...
	public function listfiles()
	{
		        ...
		        $upload_path_url = base_url() . 'files/events/';

		        $config['upload_path'] = FCPATH . 'files/events/';
		        ...
		        $this->load->library('upload', $config);
		            ...
		            $array=$this->events_model->getAllOrder()->result_array();
		            //var_dump($array);
...
/*		            $foundFiles = array();
	...
	public function do_upload2()
	{
			...
			if(is_dir(FCPATH."files/events") &&
				is_writable(FCPATH."files/events/") &&
				is_dir(FCPATH."files/events/thumbs/") &&
				is_writable(FCPATH."files/events/thumbs/")
				)
			{
				...
				else
		        {
		            ...
		            $info->deleteUrl = base_url() . 'events/deleteImage/' . $data['file_name'];
		            $info->modifyUrl = base_url() . 'events/updateImage/' . $data['file_name'];
		            $info->deleteType = 'DELETE';
		            $info->title=$titlephoto;
		            $info->description=$desc;

		           //s $info->error = $this->upload->display_errors();
		            $this->load->model("events_model");
		           $res= $this->events_model->insert(array('title'=>$titlephoto[0],"description"=>$desc[0],"size"=>$data['file_size'],'name'=>$data['file_name'],"url"=>$info->url,
	...
	{
		//echo "photo".$photo. "field ".$field." id".$id;
		//exit;
		$array=array($field=>$this->input->post('value'));
		$this->load->model("events_model");
		if($this->events_model->update($id,$array,'id','id_album'))
		{
			$d=$this->events_model->getOnePhoto($field,$id);
	...
	public function deleteImage($file)//gets the job done but you might want to add error checking and security
	{
		...
		$this->events_model->deleteByFileName($file);
		$success =unlink(FCPATH.'files/events/' .$file);
		//info to see if it is doing what it is supposed to
		$info->sucess =$success;
		$info->path =base_url().'files/events/' .$file;
		$info->file =is_file(FCPATH.'files/events'.$file);
	...
	public function sort()
	{
			...
			foreach ($ids as $key => $value) {
					$ret=$this->events_model->updateOrder($value,$key,$id_album);
