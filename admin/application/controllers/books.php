...
class Books extends CI_Controller {
	...
	public function listfiles()
	{
		        ...
		        $upload_path_url = base_url() . 'files/books/';

		        $config['upload_path'] = FCPATH . 'files/books/';
		        ...
		        $this->load->library('upload', $config);
		            ...
		            $array=$this->books_model->getAllOrder()->result_array();
		            //var_dump($array);
...
/*		            $foundFiles = array();
	...
	public function do_upload2()
	{
			...
			if(is_dir(FCPATH."files/books") &&
				is_writable(FCPATH."files/books/") &&
				is_dir(FCPATH."files/books/thumbs/") &&
				is_writable(FCPATH."files/books/thumbs/")
				)
			{
				...
				else
		        {
		            ...
		            $info->deleteUrl = base_url() . 'books/deleteImage/' . $data['file_name'];
		            $info->modifyUrl = base_url() . 'books/updateImage/' . $data['file_name'];
		            ...
		            $info->error = $this->upload->display_errors();
		            $this->load->model("books_model");
		           $res= $this->books_model->insert(array('title'=>$titlephoto[0],"description"=>$desc[0],"size"=>$data['file_size'],'name'=>$data['file_name'],"url"=>$info->url,
	...
	public function updateImage($photo,$field,$id)
	{
		...
		$this->load->model("books_model");
		if($this->books_model->update($id,$array,'id','id_album'))
		{
			$d=$this->books_model->getOnePhoto($field,$id);
	...
	public function deleteImage($file)//gets the job done but you might want to add error checking and security
	{
		...
		$this->books_model->deleteByFileName($file);
		$success =unlink(FCPATH.'files/books/' .$file);
		//info to see if it is doing what it is supposed to
		$info->sucess =$success;
		$info->path =base_url().'files/books/' .$file;
		$info->file =is_file(FCPATH.'files/books'.$file);
	...
	public function sort()
	{
			...
			foreach ($ids as $key => $value) {
					$ret=$this->books_model->updateOrder($value,$key,$id_album);
