...
class Init extends CI_Controller {
	...
	public function index()
	{
		...
		$data['content']="init/edit";
		$data['pg']="init";
		$data['title']="Fotos de Inicio";
	...
	public function listfiles()
	{
		        ...
		        $upload_path_url = base_url() . 'files/init/';

		        $config['upload_path'] = FCPATH . 'files/init/';
		        ...
		        $this->load->library('upload', $config);
		            ...
		            $array=$this->init_model->getAllOrder()->result_array();
		            //var_dump($array);
...
/*		            $foundFiles = array();
	...
	public function do_upload2()
	{
			...
			if(is_dir(FCPATH."files/init") &&
				is_writable(FCPATH."files/init/") &&
				is_dir(FCPATH."files/init/thumbs/") &&
				is_writable(FCPATH."files/init/thumbs/")
				)
			{
				...
				else
		        {
		            ...
		            $info->deleteUrl = base_url() . 'init/deleteImage/' . $data['file_name'];
		            $info->modifyUrl = base_url() . 'init/updateImage/' . $data['file_name'];
		            ...
		            $info->error = $this->upload->display_errors();
		            $this->load->model("init_model");
		           $res= $this->init_model->insert(array('title'=>$titlephoto[0],"description"=>$desc[0],"size"=>$data['file_size'],'name'=>$data['file_name'],"url"=>$info->url,
	...
	public function updateImage($photo,$field,$id)
	{
		...
		$this->load->model("init_model");
		if($this->init_model->update($id,$array,'id','id_album'))
		{
			$d=$this->init_model->getOnePhoto($field,$id);
	...
	public function deleteImage($file)//gets the job done but you might want to add error checking and security
	{
		...
		$this->init_model->deleteByFileName($file);
		$success =unlink(FCPATH.'files/init/' .$file);
		//info to see if it is doing what it is supposed to
		$info->sucess =$success;
		$info->path =base_url().'files/init/' .$file;
		$info->file =is_file(FCPATH.'files/init'.$file);
	...
	public function sort()
	{
			...
			foreach ($ids as $key => $value) {
					$ret=$this->init_model->updateOrder($value,$key,$id_album);
