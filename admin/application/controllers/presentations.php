...
public function __construct() {
	...
	public function listfiles()
	{
		        ...
		        $upload_path_url = base_url() . 'files/presentations/';

		        $config['upload_path'] = FCPATH . 'files/presentations/';
		        ...
		        $this->load->library('upload', $config);
		            ...
		            $array=$this->presentations_model->getAllOrder()->result_array();
		            //var_dump($array);
...
/*		            $foundFiles = array();
	...
	public function do_upload2()
	{
			...
			if(is_dir(FCPATH."files/presentations") &&
				is_writable(FCPATH."files/presentations/") &&
				is_dir(FCPATH."files/presentations/thumbs/") &&
				is_writable(FCPATH."files/presentations/thumbs/")
				)
			{
				...
				else
		        {
		            ...
		            $info->deleteUrl = base_url() . 'presentations/deleteImage/' . $data['file_name'];
		            $info->modifyUrl = base_url() . 'presentations/updateImage/' . $data['file_name'];
		            ...
		            $info->error = $this->upload->display_errors();
		            $this->load->model("presentations_model");
		           $res= $this->presentations_model->insert(array('title'=>$titlephoto[0],"description"=>$desc[0],"size"=>$data['file_size'],'name'=>$data['file_name'],"url"=>$info->url,
	...
	public function updateImage($photo,$field,$id)
	{
		...
		$this->load->model("presentations_model");
		if($this->presentations_model->update($id,$array,'id','id_album'))
		{
			$d=$this->presentations_model->getOnePhoto($field,$id);
	...
	public function deleteImage($file)//gets the job done but you might want to add error checking and security
	{
		...
		$this->presentations_model->deleteByFileName($file);
		$success =unlink(FCPATH.'files/presentations/' .$file);
		//info to see if it is doing what it is supposed to
		$info->sucess =$success;
		$info->path =base_url().'files/presentations/' .$file;
		$info->file =is_file(FCPATH.'files/presentations'.$file);
	...
	public function sort()
	{
			...
			foreach ($ids as $key => $value) {
					$ret=$this->presentations_model->updateOrder($value,$key,$id_album);
...
/* End of file presentations.php */
/* Location: ./application/controllers/presentations.php */